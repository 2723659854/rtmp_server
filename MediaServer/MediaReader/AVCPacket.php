...
class AVCPacket
{
    ...
    可以把视频的压缩和传输过程比作搬家，首先将待搬的东西大致分类并装进不同的箱子里，然后贴上内容标签，再逐个箱子搬走。NALU 就好比这个过程中贴了标签的箱子，是一种能够表示内部数据类型的语法结构，也是视频数据存放和传输的基本单元。
    NALU 数据常表现为 RBSP（字节流）的形式，RBSP 则表示箱子的体积信息。在 H265 中，NALU 不仅可以表示视频参数集（VPS）、序列参数集（SPS）、图像参数集（PPS）等，还可以表示补充增强信息（SEI）等。
