...
请先启动php server.php 服务，等待命令行提示`关键帧存入完毕,一共=500帧`的时候，启动代理服务器php flv.php。代理服务器可以转发flv格式直播数据。
但是目前存在问题。<br>
1，掉帧严重，你会发现画面有很多马赛克，重影。<br>
2，由于设计缺陷，代理客户端保存了所有的数据帧，导致内存过载。最终导致代理客户端死机。<br>
3，没有对各路直播分流转发。如果多路直播推流，会导致数据错乱。这个简单，后面再弄。<br>
4，直播数据转发不稳定。<br>
以后再来修复这些问题吧。
