...
class HlsDemo3
{
    ...
    public static function make(MediaFrame $frame, string $playStreamPath)
    {
        ...
        /** 写入pat包，指定pmt的pid */
        $patPacket = self::createPatPacket(4096);
        fwrite($fileHandle, $patPacket);
        /** 写入pmt包，指定音视频的pid */
        $pmtPacket = self::createPmtPacket(256, 256, 257);
        fwrite($fileHandle, $pmtPacket);
        /** 写入nit包，实际使用中根据需要填写 */
        $nitPacket = self::createNitPacket();
        fwrite($fileHandle, $nitPacket);
        /** 生成ts文件 */
        foreach ($mediaData as $frame) {
            $pid = $frame->FRAME_TYPE == MediaFrame::AUDIO_FRAME ? 257 : 256;
            $payload = $frame->_data;
            $pts = $frame->timestamp;
            $dts = $frame->timestamp;
            $pesPacket = self::createPes(224, $payload, $pts, $dts);
            self::writeTsPacket($pid, $pesPacket, $fileHandle, $continuity_counter, 1, 1);
        }
        fclose($fileHandle);
        /** 将ts文件追加到目录 */
        $tsFiles[] = $tsFile;
    ...
    public static function crc32_mpeg($data)
    {
        static $crc_table = array(
            ...
            0x4C11DB70, 0x48D0C6C7, 0x4593E01E, 0x4152FDB9, 0x5F15ADBC, 0x5BD4B00B, 0x569796D2, 0x52568B65,
            0x6A1936D8, 0x6ED82B6F, 0x639B0DB6, 0x675A1001, 0x791D4004, 0x7DDC5DB3, 0x709F7B6A, 0x745E66DD,
        ...
        $len = strlen($data);
        for ($i = 0; $i < $len; $i++) {
            $crc = (($crc << 8) & 0xFFFFFFFF) ^ $crc_table[(($crc >> 24) & 0xFF) ^ ord($data[$i])];
        }
        return pack('N', $crc);
    }
}
